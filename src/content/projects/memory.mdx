...
A minimal number memory training game: it shows you a sequence of digits for a few seconds, then asks you to recall them. Each level adds more digits. Spent a day building it with KMM + Compose on all platforms, running natively on Android, iOS, and in the browser as a compiled Wasm app. It was easy.
...
At the time of writing this, i'd consider it done (at least to MVP standards). In total (spread over a few different days) i spent a little less than an average work day (~8 hours) writing the code.
